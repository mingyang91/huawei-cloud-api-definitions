...
fn companie_rs_gen<P: AsRef<Path>>(schema: P, reference: &str) -> String {
    let schema = start_from_src(schema);
    format!(
        r#"
...
async fn render_cargo_toml(name: &str, features: &[String]) -> Result<String, Error> {
	...
	*pkg_name = TomlValue::String(new_name.to_string());
	...
	let new_name = format!("{}_{}", common_name, normalize_identifier(name));
...
fn start_from_src<P: AsRef<Path>>(path: P) -> std::path::PathBuf {
    let path = path.as_ref();
    let mut new_path = std::path::PathBuf::from("");
    let tails = path.components()
        .skip_while(|x| x.as_os_str() != "src");
    for tail in tails {
        new_path = new_path.join(tail);
    }
    new_path
}

#[test]
fn start_from_src_test() {
    let path = Path::new("/tmp/abc/def/src/ghi.json");
    let res = start_from_src(path);
    dbg!(res);
}